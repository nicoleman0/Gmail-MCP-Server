...
This section presents a detailed security analysis of the Gmail MCP Server (`@gongrzhe/server-gmail-autoauth-mcp` v1.1.11), an open-source MCP server that exposes 19 Gmail operations as LLM-callable tools. With an estimated 117,000 total downloads and approximately 2,200 weekly downloads (PulseMCP, February 2026), and 822 GitHub stars, it is one of the most widely-deployed Gmail MCP servers in the ecosystem, operating in a high-value domain: personal and corporate email. The server was selected as a case study because it combines a rich attack surface (read, write, delete, forward email; filesystem access; OAuth credentials) with the characteristic MCP challenge of processing untrusted content (email bodies) that flows through LLM context.
...
The scale of exposure is significant: with approximately 117,000 total downloads, any user who followed the cloud server installation path has had their Gmail authorization code routed through `gmail.gongrzhe.com`. Intercepted refresh tokens provide persistent, silent access to Gmail that survives password changes and does not appear in the user's active session list.

Cross-server comparison provides additional context. The same author's Google Calendar MCP server (`@gongrzhe/server-calendar-autoauth-mcp`) implements an identical authentication flow but uses only `http://localhost:3000/oauth2callback` — no external domain is documented or referenced anywhere in its README or source. This comparison is significant: it establishes that the developer was not architecturally constrained to use an external callback domain, and that the Gmail server's use of `gmail.gongrzhe.com` represented a deliberate choice rather than a default pattern applied uniformly across his servers.

The rationale for that choice likely relates to the server's documented n8n integration use case. n8n is a self-hosted workflow automation platform whose users commonly run it on VPS or cloud infrastructure where localhost OAuth callbacks are not viable. The Gmail server's README specifically mentions n8n as the target environment for the external callback flow. While this explains the motivation for providing an external callback endpoint, it does not mitigate the security consequence: centralising all cloud deployment OAuth flows through a single developer-controlled domain creates an inherent trust dependency, and domain lapse converts that dependency into an open attack surface.
...
As of the date of this writing, no response has been received. The author's broader publication pattern is relevant context: the GongRzhe GitHub account and npm namespace contain dozens of MCP servers published in rapid succession over a short period, suggesting automated or semi-automated development rather than sustained maintenance engagement. The absence of a response to a coordinated disclosure of a critical credential theft vulnerability — combined with the domain lapse and the continued presence of the vulnerable documentation in the published README — means the attack surface described in F01 remains open.

Full public disclosure of F01 specifics will proceed in accordance with standard responsible disclosure timelines following a reasonable opportunity for the author to respond.
